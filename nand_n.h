...
public:
	...
	void prc_Original_nand_n(){
		...
		if (accessRegistry->getObjectFaultType(hardwareObjectId,1) == NoFault 
			...
			if (accessRegistry->getObjectFaultType(hardwareObjectId,1) == SA0 || accessRegistry->getObjectFaultType(hardwareObjectId,2) == SA0)
				out1 = SC_LOGIC_1;
			
			else if (accessRegistry->getObjectFaultType(hardwareObjectId,1) == SA1 
					&& accessRegistry->getObjectFaultType(hardwareObjectId,2) == SA1)
				out1 = SC_LOGIC_0;
			...
			else if (accessRegistry->getObjectFaultType(hardwareObjectId,2) == SA1){
