...
SC_MODULE(flt_injector){

    sc_export<sc_signal<sc_logic>> i0, i1, ci, s_gdut, co_gdut, s_fdut, co_fdut;
    // sc_out< sc_logic > i0, i1, ci;

    faultRegistry* accessRegistry;

    // SC_CTOR(flt_injector){
	SC_HAS_PROCESS(flt_injector);
	flt_injector(sc_module_name _name){
